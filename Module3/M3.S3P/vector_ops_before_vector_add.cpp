...
// this variable is the read buffer for getting results from the device
cl_mem bufV;
...
int main(int argc, char **argv)
{
    ...
    //  global size to size of SZ's type - which is int
    ...
    // bufV is buffer being read from
    // CL_TRUE is for blocking_read or not - blocks until done
    // 0 is offset
    // SZ * sizeof(int) is size of data being read from bufV
    // &v[0] is for ptr - pointer to be written to
...
void setup_kernel_memory()
{
    ...
    // SZ * sizeof(int) is size of the buffer
